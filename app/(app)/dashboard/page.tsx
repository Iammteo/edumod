...
export default async function DashboardPage() {
  ...
  if (role === "admin") {
    ...
    return (
      <AdminApp
        userName={name}
        school={{ name: school?.name ?? schoolName, schoolCode, email: school?.email ?? null, phone: school?.phone ?? null, state: school?.state ?? null, country: school?.country ?? null, address: school?.address ?? null, logoKey: school?.logoKey ?? null, requireApproval: !!school?.requireApproval }}
        ...
        staff={staffRows.map((s) => ({ userId: s.userId, name: s.name, email: s.email, role: s.role, teacherType: s.isClassTeacher ? "Class teacher" : s.isTeacher ? "Subject teacher" : (ROLE_LABEL[s.role] ?? "Staff"), subjects: (s.subjects as string[]) ?? [], assignedClass: s.assignedClass ?? null, status: s.status ?? "active", canApprove: !!s.canApprove, permissions: (s.permissions as Record<string, string>) ?? {} }))}
        audit={auditRows.map((a) => ({ action: a.action, entityType: a.entityType, actor: a.actorName ?? "system", actorRole: a.actorRole ? (ROLE_LABEL[a.actorRole] ?? (a.actorRole === "school_admin" ? "Admin" : a.actorRole)) : null, ts: new Date(a.createdAt).getTime(), detail: describeAudit(a.action, a.metadata as Record<string, unknown>), meta: a.metadata as Record<string, unknown> }))}
