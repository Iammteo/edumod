...
export type School = { name: string; schoolCode: string; email: string | null; phone: string | null; state: string | null; country: string | null; address: string | null; logoKey: string | null; requireApproval: boolean };
type Student = { id: string; name: string; admissionNo: string; createdAt: string; className?: string | null };
export type Staff = { userId: string; name: string; email: string | null; role: string; teacherType: string; subjects: string[]; assignedClass: string | null; status: string; canApprove: boolean; permissions: Record<string, string> };
type Audit = { action: string; entityType: string; actor: string | null; actorRole?: string | null; ts: number; detail: string; meta?: Record<string, unknown> };
    ...
    <div className="grid min-h-screen bg-paper lg:grid-cols-[244px_1fr]">
      ...
      <div className="flex min-w-0 flex-col">
        <header className="flex items-center justify-between gap-3 border-b border-border-soft bg-white px-4 py-3 lg:hidden">
          ...
          <NotifBell open={notifOpen} setOpen={setNotifOpen} audit={audit} onViewAll={() => setActive("audit")} />
        </header>

        <main className="overflow-x-hidden p-4 sm:p-6 lg:px-7 lg:py-6">
          {active === "overview" && <Overview userName={userName} students={students} staff={staff} audit={audit} overview={overview} notifOpen={notifOpen} setNotifOpen={setNotifOpen} goto={setActive} />}
...
function NotifBell({ open, setOpen, audit, onViewAll }: { open: boolean; setOpen: (v: boolean) => void; audit: Audit[]; onViewAll: () => void }) {
  ...
  return (
    <div className="relative">
      ...
      {open && <><div className="fixed inset-0 z-40" onClick={() => setOpen(false)} /><div className="absolute right-0 top-12 z-50 w-[min(320px,calc(100vw-32px))] rounded-2xl border border-border-soft bg-white p-2 shadow-[0_20px_50px_rgba(16,33,63,.16)] motion-safe:animate-[fade-up_.2s_ease]"><div className="flex items-center justify-between px-2 py-1.5"><strong className="text-[13px]">Notifications</strong>{items.length > 0 && <button onClick={markRead} className="text-[11px] font-bold text-brand-blue hover:underline">Mark all as read</button>}</div>
        {items.length === 0 ? <p className="px-2 py-6 text-center text-[12px] text-ink-soft">You&rsquo;re all caught up.</p> : items.map((a, i) => { const isNew = a.ts > baseline; return <div key={i} className={`flex gap-2.5 rounded-xl px-2 py-2 ${isNew ? "bg-brand-soft/40" : "hover:bg-paper/70"}`}><span className="relative grid size-7 shrink-0 place-items-center rounded-lg bg-brand-soft text-[12px] font-bold text-brand-blue">●{isNew && <span className="absolute -right-0.5 -top-0.5 size-2 rounded-full bg-brand-green ring-2 ring-white" />}</span><div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><strong className="truncate text-[11px]">{a.detail || actLabel(a.action)}</strong><span className="shrink-0 text-[10px] text-ink-soft">{relTime(a.ts)}</span></div><p className="text-[10px] leading-snug text-ink-soft">{actLabel(a.action)} · {a.actor ?? "system"}</p></div></div>; })}
...
function Overview({ userName, students, staff, audit, overview, notifOpen, setNotifOpen, goto }: { userName: string; students: Student[]; staff: Staff[]; audit: Audit[]; overview: AdminOverview; notifOpen: boolean; setNotifOpen: (v: boolean) => void; goto: (s: string) => void }) {
  ...
  return (
    <>
      <Head title={`Good morning, ${userName.split(" ")[0]} 👋`} subtitle="Here's what's happening in your school today." action={<div className="flex items-center gap-2.5"><div className="hidden sm:block"><NotifBell open={notifOpen} setOpen={setNotifOpen} audit={audit} onViewAll={() => goto("audit")} /></div><Select>2023/2024 · Term 2</Select></div>} />
      <div className="grid gap-3.5 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map(([label, value, meta, color, bg, icon, target]) => (
          <button key={label} onClick={() => goto(target)} className="rounded-2xl border border-border-soft bg-white p-[18px] text-left transition hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(16,33,63,.08)]">
            <div className="flex items-start justify-between"><div><small className="font-bold text-ink-soft">{label}</small><strong className="mt-2 block break-words font-display text-[clamp(20px,5vw,27px)] font-semibold leading-none">{value}</strong></div><span className="grid size-9 shrink-0 place-items-center rounded-full" style={{ backgroundColor: bg, color }}>{I(icon)}</span></div>
            <span className="mt-3 inline-block text-[11px] font-extrabold text-ink-soft">{meta}</span>
          </button>
        ))}
      </div>
      <div className="mt-[18px] grid gap-[18px] xl:grid-cols-[1.45fr_1fr]">
        <Card title="Fee collection (last 6 months)">
          {hasSeries ? <BarChart yMax={Math.max(...overview.series.map((p) => p.value)) * 1.2 || 1000} yLabel={(n) => (n >= 1_000_000 ? `${(n / 1_000_000).toFixed(0)}M` : `${Math.round(n / 1000)}k`)} data={overview.series} />
            : <Empty text="No approved payments yet. Collections will chart here as payments are approved." />}
        </Card>
        <Card title="Students by section" action={<button onClick={() => goto("students")} className="text-[11px] font-extrabold text-brand-blue hover:underline">View students</button>}>
      ...
      <div className="mt-[18px]">
        <Card title="Recent activity" action={<button onClick={() => goto("audit")} className="text-[11px] font-extrabold text-brand-blue hover:underline">View all</button>}>
          {audit.length === 0 ? <Empty text="No activity yet. Actions in your school will appear here." /> : audit.slice(0, 6).map((a, i) => <div key={i} className="flex gap-2.5 border-b border-border-soft py-2.5 last:border-0"><span className="grid size-7 shrink-0 place-items-center rounded-lg bg-brand-soft text-[11px] font-bold text-brand-blue">●</span><div className="min-w-0"><strong className="block text-[11px]">{a.detail || actLabel(a.action)}</strong><span className="text-[10px] text-ink-soft">{actLabel(a.action)} · by {a.actor ?? "system"} · {relTime(a.ts)}</span></div></div>)}
        </Card>
      </div>
    </>
  );
...
function Students({ students }: { students: Student[] }) {
  ...
  return (
    <>
      <Head title="Students" subtitle={`${students.length.toLocaleString()} student${students.length === 1 ? "" : "s"} enrolled`} action={<PrimaryBtn onClick={() => setShowAdd((s) => !s)}>{I(<><path d="M12 5v14M5 12h14" /></>)}{showAdd ? "Close" : "Add student"}</PrimaryBtn>} />
...
function Settings({ school, onSaved }: { school: School; onSaved: (s: School) => void }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const input = { name: String(fd.get("name") || ""), email: String(fd.get("email") || ""), phone: String(fd.get("phone") || ""), state: String(fd.get("state") || ""), country: String(fd.get("country") || ""), address: String(fd.get("address") || ""), requireApproval };
    setBusy(true); setMsg(null);
