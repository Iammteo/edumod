...
export function DashboardChrome({ roleLabel, school, schoolCode, term, userName, title, subtitle, nav, children }: { roleLabel: string; school: string; schoolCode: string; term: string; userName: string; title: string; subtitle: string; nav: string[]; children: React.ReactNode }) {
  ...
  const side = (
    <>
      ...
      <nav className="grid">{nav.map((name, i) => <a key={name} href={`#${name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`} onClick={() => setOpen(false)} className={`my-0.5 rounded-[9px] px-3 py-2.5 text-[13px] font-bold transition ${i === 0 ? "bg-sidebar-active text-white" : "text-sidebar-faint hover:bg-white/10"}`}>{name}</a>)}</nav>
      <div className="mt-auto border-t border-white/15 pt-[15px]">
        <div className="text-[11px] text-[#d8e3f3]">{userName}<br /><span className="text-sidebar-muted">{roleLabel}</span></div>
