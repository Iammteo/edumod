...
const statusPill = (s: string) => s === "cleared" ? ["Cleared", "bg-brand-green/10 text-brand-green"] : s === "at_risk" ? ["Exam at risk", "bg-danger-soft text-danger"] : ["Needs follow-up", "bg-warn-soft text-warn"];
const stuPill = (s: string) => s === "cleared" ? ["Cleared", "bg-brand-green/10 text-brand-green"] : s === "partial" ? ["Partial payment", "bg-warn-soft text-warn"] : ["Unpaid", "bg-danger-soft text-danger"];
...
export function ClassFinanceView({ onViewInvoices }: { onViewInvoices?: () => void }) {
  ...
  if (!data) return err ? <LoadError message={err} onRetry={load} /> : <LoadingPanel stats={6} />;
  const t = data.totals;
  const stats = [
  ...
  return (
    <>
      <div className="mb-5 flex flex-col justify-between gap-3 lg:flex-row lg:items-start">
        <div><div className="text-[11px] font-extrabold text-brand-blue">Finance overview</div><h1 className="font-display text-[clamp(21px,3.5vw,28px)] font-semibold leading-tight">Class finance summary</h1><p className="mt-0.5 text-[13px] text-ink-soft">Track fee performance for each class and spot outstanding balances quickly.</p></div>
            ...
            <>
              ...
              <ul className="grid gap-2 md:hidden">{filtered.map((c) => { const [sl, sc] = statusPill(c.status); const sel = selected === c.className; return (
                <li key={c.className}><button type="button" onClick={() => setSelected(c.className)} className={`w-full rounded-xl border p-3 text-left transition ${sel ? "border-brand-blue bg-brand-soft/40" : "border-border-soft"}`}>
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-2 text-[11px]"><div><div className="text-[9px] uppercase tracking-wide text-ink-soft">Students</div><div className="font-bold text-ink">{c.students}</div></div><div><div className="text-[9px] uppercase tracking-wide text-ink-soft">Paid</div><div className="font-bold text-ink">{naira(c.collected)}</div></div><div><div className="text-[9px] uppercase tracking-wide text-ink-soft">Outstanding</div><div className="font-extrabold text-danger">{naira(c.outstanding)}</div></div></div>
              ...
              <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[640px] text-left text-[12px]">
                ...
                <tbody>{filtered.map((c) => { const [sl, sc] = statusPill(c.status); const sel = selected === c.className; return (
                  <tr key={c.className} onClick={() => setSelected(c.className)} className={`cursor-pointer border-b border-border-soft last:border-0 ${sel ? "bg-brand-soft/50" : "hover:bg-paper/60"}`}>
                    ...
                    <td className="py-2.5 font-extrabold text-danger">{naira(c.outstanding)}</td>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[.8fr_1.6fr]">
        <section className="rounded-2xl border border-border-soft bg-white p-5">
          <h2 className="mb-3 flex items-center gap-1.5 font-display text-[15px] font-semibold"><span className="text-danger">⚠</span>Top outstanding classes</h2>
          {top.length === 0 ? <p className="text-[12px] text-ink-soft">No outstanding balances. 🎉</p> : <div className="grid gap-2.5">{top.map((c) => <button key={c.className} onClick={() => setSelected(c.className)} className="grid gap-1 text-left"><div className="flex items-center justify-between text-[11px]"><span className="font-bold text-ink">{c.className}</span><span className="font-extrabold text-danger">{naira(c.outstanding)}</span></div><span className="h-2 overflow-hidden rounded-full bg-paper"><span className="block h-full rounded-full bg-danger-strong" style={{ width: `${Math.round((c.outstanding / maxTop) * 100)}%` }} /></span></button>)}</div>}
        </section>
            ...
            <>
              ...
              <ul className="grid gap-2 md:hidden">{studentsFiltered.slice(0, 12).map((s) => { const [sl, sc] = stuPill(s.status); return (
                <li key={s.id} className="rounded-xl border border-border-soft p-3">
                  <div className="flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2.5"><span className="grid size-7 shrink-0 place-items-center rounded-full text-[10px] font-extrabold text-white" style={{ backgroundColor: AV[s.name.length % AV.length] }}>{initials(s.name)}</span><StudentLink studentId={s.id} name={s.name} className="truncate font-bold text-ink" /></div><span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-extrabold ${sc}`}>{sl}</span></div>
                  ...
                  <div className="mt-1.5 flex items-center justify-between text-[12px]"><span className="text-ink-soft">Paid <b className="text-ink">{naira(s.paid)}</b></span><span className="text-ink-soft">Due <b className={s.outstanding > 0 ? "text-danger" : "text-brand-green"}>{naira(s.outstanding)}</b></span></div>
                </li>); })}</ul>
              {/* Desktop: table */}
              <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[560px] text-left text-[12px]">
                ...
                <tbody>{studentsFiltered.slice(0, 12).map((s) => { const [sl, sc] = stuPill(s.status); return (
                  <tr key={s.id} className="border-b border-border-soft last:border-0 hover:bg-paper/60">
                    <td className="py-2.5"><div className="flex items-center gap-2.5"><span className="grid size-7 shrink-0 place-items-center rounded-full text-[10px] font-extrabold text-white" style={{ backgroundColor: AV[s.name.length % AV.length] }}>{initials(s.name)}</span><StudentLink studentId={s.id} name={s.name} className="font-bold text-ink" /></div></td>
                    ...
                    <td className={`py-2.5 font-extrabold ${s.outstanding > 0 ? "text-danger" : "text-brand-green"}`}>{naira(s.outstanding)}</td>
                    <td className="py-2.5"><span className={`rounded-full px-2 py-0.5 text-[10px] font-extrabold ${sc}`}>{sl}</span></td>
  ...
  return (
    <section className="rounded-2xl border border-border-soft bg-white p-5">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        {[["Students", String(detail.students)], ["Expected", naira(detail.expected)], ["Paid", naira(detail.collected)], ["Outstanding", naira(detail.outstanding)]].map(([k, v], i) => <div key={k}><div className="text-[10px] font-bold uppercase tracking-wide text-ink-soft">{k}</div><div className={`mt-0.5 font-display text-[clamp(13px,3vw,16px)] font-bold leading-none ${i === 3 ? "text-danger" : "text-ink"}`}>{v}</div></div>)}
      </div>
      <div className="mt-4 grid items-center gap-4 sm:grid-cols-[1fr_auto]">
        ...
        <div className="mx-auto grid place-items-center">
          ...
          <div className="mt-2 flex gap-3 text-[10px] font-bold"><span className="flex items-center gap-1 text-brand-green"><span className="size-2 rounded-full bg-brand-green" />Paid</span><span className="flex items-center gap-1 text-danger"><span className="size-2 rounded-full bg-danger-strong" />Outstanding</span></div>
        </div>
      </div>
    </section>
  );
